...
class CompanyData(FMPReader):
    ...
    def _get_data(self, req_type=None, period=None):
        ...
        dict_ = self._get_fields(req_type, subfield)
        if period == "both":
            annual = dict2df(self._send_request(req_type, period='annual', subfield=subfield), dict_, self._data_type)
            quarter = dict2df(self._send_request(req_type, period='quarter', subfield=subfield), dict_, self._data_type)
            annual['Period'] = 'A'
            quarter['Period'] = 'Q'
            reports = annual.append(quarter).sort_values(by=['date'], ascending=False)
        else:
            reports = dict2df(self._send_request(req_type, period=period, subfield=subfield), dict_, self._data_type)
